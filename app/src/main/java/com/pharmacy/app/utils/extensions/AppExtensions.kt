...
import com.pharmacy.app.PharmacyApp
import com.pharmacy.app.PharmacyApp.Companion.getAppInstance
...
import com.pharmacy.app.utils.Constants.PLAY_STORE_URL_PREFIX
...
import com.pharmacy.app.utils.Constants.SharedPref.IS_LOGGED_IN
import com.pharmacy.app.utils.Constants.SharedPref.IS_SOCIAL_LOGIN
import com.pharmacy.app.utils.Constants.SharedPref.KEY_ADDRESS
import com.pharmacy.app.utils.Constants.SharedPref.KEY_CART_COUNT
import com.pharmacy.app.utils.Constants.SharedPref.KEY_DASHBOARD
import com.pharmacy.app.utils.Constants.SharedPref.KEY_ORDERS
import com.pharmacy.app.utils.Constants.SharedPref.KEY_RECENTS
import com.pharmacy.app.utils.Constants.SharedPref.KEY_USER_ADDRESS
...
import com.pharmacy.app.utils.Constants.SharedPref.USER_DISPLAY_NAME
import com.pharmacy.app.utils.Constants.SharedPref.USER_EMAIL
import com.pharmacy.app.utils.Constants.SharedPref.USER_FIRST_NAME
import com.pharmacy.app.utils.Constants.SharedPref.USER_ID
import com.pharmacy.app.utils.Constants.SharedPref.USER_LAST_NAME
import com.pharmacy.app.utils.Constants.SharedPref.USER_NICE_NAME
import com.pharmacy.app.utils.Constants.SharedPref.USER_PROFILE
import com.pharmacy.app.utils.Constants.SharedPref.USER_ROLE
import com.pharmacy.app.utils.Constants.SharedPref.USER_TOKEN
import com.pharmacy.app.utils.Constants.SharedPref.USER_USERNAME
import com.pharmacy.app.utils.Constants.SharedPref.WISHLIST_DATA
...
fun getAppLanguage(): String = getSharedPrefInstance().getStringValue(LANGUAGE, defaultValue = "ru")
...
fun getSharedPrefInstance(): SharedPrefUtils {
    return if (PharmacyApp.sharedPrefUtils == null) {
        PharmacyApp.sharedPrefUtils = SharedPrefUtils()
        PharmacyApp.sharedPrefUtils!!
    } else {
        PharmacyApp.sharedPrefUtils!!
    }
}
...
fun Activity.openLottieDialog(jsonFileCode: JsonFileCode = JsonFileCode.NO_INTERNET, onLottieClick: () -> Unit)  {
    try{
        ...
        if (noInternetDialog == null) {
            ...
            noInternetDialog?.findViewById<View>(R.id.rlLottie)?.onClick {
        ...
        noInternetDialog?.findViewById<com.airbnb.lottie.LottieAnimationView>(R.id.lottieNoInternet)?.setAnimation(jsonFile)
...
fun BaseActivity.getOrders(page: Int = 1, onApiSuccess: (ArrayList<MyOrderData>) -> Unit) {
...
fun BaseActivity.createCustomer(requestModel: RequestModel, onApiSuccess: (LoginData) -> Unit) {
    showProgress(true)
...
fun BaseActivity.socialLogin(email:String,accessToken: String, firstName: String, lastName: String, loginType: String, photoURL: String, onResult: (Boolean) -> Unit, onError: (String) -> Unit) {
    val requestModel = RequestModel()
...
fun BaseActivity.processPayment(requestModel: RequestModel, isContainRedirectUrl: Boolean = true) {
    showProgress(true)
...
fun BaseActivity.updateItem(product: CartResponse) {
...
fun BaseActivity.getCartTotal(): Int? {
    val list = getCartListFromPref()
    var count = 0
    ...
    findViewById<TextView>(R.id.tvOffer)?.text = getString(R.string.text_offer_not_available)
    findViewById<TextView>(R.id.tvShippingCharge)?.text = getString(R.string.lbl_free)
    findViewById<TextView>(R.id.tvTotalAmount)?.text = count.toString().currencyFormat()
    return count
}
...
fun isExistInWishList(productId: Int): Boolean {
    if (getSharedPrefInstance().getStringValue(WISHLIST_DATA) == "") {
        return false
    }
...
fun setProductItem(view: View, item: ProductDataNew) {
    ...
    if (item.sale_price!!.isNotEmpty()) {
        tvDiscountPrice?.text = item.sale_price.currencyFormat()
        tvOriginalPrice?.text = item.regular_price?.currencyFormat()
        tvOriginalPrice?.applyStrike()
    } else {
        tvOriginalPrice?.text = ""
        tvDiscountPrice?.text = item.price?.currencyFormat()
    }
    ratingBar?.rating = item.average_rating!!.toFloat()

    if (item.full != null) ivProduct?.loadImageFromUrl(item.full)
}
...
fun Activity.cancelOrder(orderId:Int, progressBar: ProgressBar? = null, onApiSuccess: (MyOrderData) -> Unit,onError: (String) -> Unit){
    val requestModel= RequestModel()
    requestModel.status="cancelled"

    callApi(getRestApis().updateOrder(orderId,requestModel), onApiSuccess = {
